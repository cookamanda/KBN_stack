...
#posterior = 1/C p(η) ∑_i ∫_S_i Pois(N_i, B_i, F_i | λ = S_i) TNorm_S^∞(2η F_i/(F_2_S), η F_i/(F_2_S)) d S_i

def S_i_integrand(S_i, N_i, B_i, a_TNorm, eta, F_i, F_2_S, F_i_err):
    log_poisson = poisson.logpmf(N_i, S_i + B_i)
    truncnorm_loc = 2 * eta * F_i / F_2_S
    truncnorm_scale = eta * F_i / F_2_S
...
def integrate_S_i_function(N_i, B_i, a_TNorm, eta, F_i, F_2_S, F_i_err):
    if isinstance(N_i, (int, float)):
        ...
        for i in range(len(N_i)):
            result[i], _ = quad(S_i_integrand, 0, np.inf, args=(N_i[i], B_i[i], a_TNorm, eta, F_i[i], F_2_S[i], F_i_err[i]))
...
def custom_integrate_S_i_function(N_i, B_i, a_TNorm, eta, F_i, F_2_S, F_i_err):
    ...
    if isinstance(N_i, (int, float)):
        ...
        result = np.zeros(len(N_i))
        for i in range(len(N_i)):
            result[i] =  np.sum(np.multiply(S_i_integrand(test_s, N_i[i], B_i[i], a_TNorm, eta, F_i[i], F_2_S[i], F_i_err[i]), bin_edges))
...
def eta_posterior(etas, binsizes, N_i, B_i, F_i, F_i_err, F_2_S, prior=None):
    ...
    for i, eta in tqdm(enumerate(etas)):
        a_TNorm = -1 #(0 - eta*F_i/F_2_S) / (F_i_err*eta/F_2_S)
